...
- (void)viewDidLoad {
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(actionChangeOrientation) name:UIDeviceOrientationDidChangeNotification object:nil];
...
- (void)setNavigationBar {
    ...
    if (SYSTEM_VERSION_LESS_THAN(@"7")) {
        [self.navigationController.navigationBar setBackgroundColor:[UIColor colorWithRed:0.525 green:0.737 blue:0.012 alpha:1]];
    } else {
        [self.navigationController.navigationBar setBarTintColor:[UIColor colorWithRed:0.525 green:0.737 blue:0.012 alpha:1]];
    }
}
...
- (void)actionChangeOrientation {
    if (isNeedRotateLastImage) {
        [self setCurrentPhotoIndex:[self numberOfPhotos] - 1];
    }
}
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:@"Готово" style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
...
- (NSInteger)lastImage {
    ...
    NSUInteger numbersPhotoInGallery = [self numberOfPhotos];
    
    if (!numbersPhotoInGallery) {
        return 0;
    }
    
    return numbersPhotoInGallery-1;
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (_photos.count > 1 && previousCurrentPage == [self lastImage]) {
        isNeedRotateLastImage = YES;
    } else {
        isNeedRotateLastImage = NO;
    }
...
- (void)updateNavigation {
    ...
    if (_navigationBarTitle || _navigationBarTitle.length) {
...
- (void)actionButtonPressed:(id)sender {
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage] && _shareContent) {
        _shareContent([photo underlyingImage], photo.caption);
    }
}
